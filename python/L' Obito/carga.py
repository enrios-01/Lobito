...
from utils import cleaning, validate_stock
from tools import cargar_en_memoria, guardar_datos
...
#Funcion que actualiza las cantidades de productos en el diccionario stock despues de cada iteracion
def actualizar_stock_inicial(stock):
    for categoria, productos in stock.items():
        for producto in productos:
            producto[2] = 10  # Asigna una cantidad inicial de 10 a cada producto

#Valida formato de fecha y mantiene la coherencia de las fechas de nacimiento, defuncion y servicio.
def validar_fecha(mensaje, fecha_maxima=None, fecha_minima=None):
    while True:
        fecha = raw_input(mensaje)
...
def carga_servicios(servicios, estadistica, stock):
    ...
    while True:
        fecha_servicio = validar_fecha("\n\t\t\t\tIngrese la fecha del servicio (DD/MM/YYYY o DD-MM-YYYY): ", fecha_maxima=datetime.now(), fecha_minima=fecha_def)
    ...
    if funeral == 1001:  # CREMACIÓN
        ...
        while True:
            ...
            try:
                urna = int(urna)
                if validate_stock(stock, 'urnas', urna, estadistica):
                    break
            except ValueError:
        ...
        while True:
            ...
            try:
                ...
                if validate_stock(stock, 'catering', catering, estadistica):
                    break
            except ValueError:
        ...
        while True:
            ...
            try:
                feretro = int(feretro)
                if validate_stock(stock, 'feretros', feretro, estadistica):
                    break
            except ValueError:
        ...
        while True:
            ...
            codigo_caravana = raw_input("\n\t\t\t\tIngrese el código de caravana: ")
            try:
                codigo_caravana = int(codigo_caravana)
                if validate_stock(stock, "autos", codigo_caravana, estadistica):
                    break
            except ValueError:
    ...
    # Agregar esta línea para guardar los datos en el archivo .dat
    ...
    for clave, valor in ficha.iteritems():
...
if __name__ == "__main__":
    print "\n\t\t\t\t--- Bienvenido al sistema de carga de servicios ---"
    while True:
        carga_servicios(servicios, estadistica, stock)
        continuar = raw_input("\n\t\t\t\t¿Desea cargar otro servicio? (s/n): ").lower()
        if continuar != "s":
            print "\n\t\t\t\t--- Fin del programa ---"
            cleaning()
            break
